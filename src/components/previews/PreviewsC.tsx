    ...
    <div className="min-h-full bg-[#f8f9fa] p-8">
      <div className="max-w-3xl mx-auto">
        ...
        <div className="grid grid-cols-3 gap-6 mt-12">
          {[
            { title: "Create", color: "#7fdfbf" },
            { title: "Shape", color: "#b19cd9" },
            { title: "Mold", color: "#87ceeb" },
          ].map((card) => (
            <div key={card.title} className="p-6 rounded-3xl transition-transform hover:scale-105" style={{ background: card.color, boxShadow: `0 8px 32px ${card.color}44, inset 0 -4px 8px rgba(0,0,0,0.05), inset 0 4px 8px rgba(255,255,255,0.4)` }}>
              ...
              <p className="text-sm text-[#333]/60">Puffy and touchable like digital Play-Doh.</p>
            </div>
          ))}
        </div>
      </div>
...
export function MaximalismPreview() {
  return (
    <div className="min-h-full p-8 relative overflow-hidden" style={{ background: "linear-gradient(135deg, #fef3c7, #fce7f3, #e0e7ff)" }}>
        ...
        <div className="grid grid-cols-3 gap-4 mt-12">
          {[
            ...
            <div key={card.title} className="p-5 rounded-lg border-2" style={{ background: card.bg, borderColor: card.border }}>
              <h3 className="text-xl font-black mb-1" style={{ color: card.border }}>{card.title}</h3>
              <p className="text-sm text-[#1e293b]/60">Rich details that reward exploration and discovery.</p>
            </div>
          ))}
        </div>
      </div>
...
export function LiquidPreview() {
  return (
    <div className="min-h-full bg-[#050505] text-white p-8 relative overflow-hidden">
      ...
      <div className="max-w-3xl mx-auto relative z-10">
        <h1 className="text-6xl font-light mb-4 tracking-tight" style={{ fontFamily: "'Space Grotesk', sans-serif" }}>
          <span className="bg-gradient-to-r from-[#06b6d4] to-[#8b5cf6] bg-clip-text text-transparent">Fluid</span><br />
          Interfaces
        </h1>
        <p className="text-white/40 text-lg mb-10 max-w-md font-light">
          Living, breathing digital surfaces. Nothing is static. Everything flows.
        </p>
        ...
        <div className="grid grid-cols-3 gap-4 mt-16">
          {["Flow", "Morph", "Pulse"].map((title) => (
            <div key={title} className="p-6 rounded-2xl border border-white/10 bg-white/5 backdrop-blur-sm">
              ...
              <p className="text-sm text-white/30">Organic motion meets digital precision.</p>
            </div>
          ))}
        </div>
      </div>
...
export function KineticTypographyPreview() {
  return (
    <div className="min-h-full bg-black text-white p-8 flex flex-col justify-center">
      <div className="max-w-3xl mx-auto">
        <div className="mb-12">
          <h1 className="text-8xl font-black leading-none tracking-tighter mb-4">
            WORDS
          </h1>
          <h1 className="text-8xl font-black leading-none tracking-tighter text-[#f43f5e] mb-4">
            MOVE
          </h1>
        ...
        <p className="text-white/30 text-lg max-w-sm">
          Typography is not just read — it's experienced. Every letter is choreographed.
        </p>

        <div className="mt-16 flex gap-12">
          {["Design", "Motion", "Impact"].map((word) => (
            <div key={word} className="group cursor-pointer">
              <span className="text-2xl font-bold text-white/20 group-hover:text-white transition-colors duration-300">{word}</span>
            </div>
          ))}
        </div>
      </div>
...
export function ScrapbookPreview() {
  return (
    <div className="min-h-full p-8 relative" style={{ background: "#fef3c7" }}>
        ...
        <div className="grid grid-cols-3 gap-6 mt-12">
          {[
            ...
            <div key={card.title} className="p-5 shadow-lg border border-[#1c1917]/10" style={{ background: card.bg, transform: `rotate(${card.rot})` }}>
              <h3 className="text-xl font-bold text-[#1c1917] mb-1" style={{ fontFamily: "'Caveat', cursive" }}>{card.title}</h3>
              <p className="text-sm text-[#92400e]" style={{ fontFamily: "'Special Elite', cursive" }}>Cut, paste, create.</p>
            </div>
          ))}
        </div>
      </div>
...
export function Y2KPreview() {
  return (
    <div className="min-h-full p-8 relative overflow-hidden" style={{ background: "linear-gradient(135deg, #e0c3fc, #8ec5fc)" }}>
      {/* Decorative stars */}
      ...
      <div className="max-w-3xl mx-auto relative z-10">
        ...
        <p className="text-lg text-[#333]/70 mb-8 max-w-md">
        ...
        <div className="grid grid-cols-3 gap-4 mt-12">
          {["Retro ✨", "Chrome 💿", "Vibes 🌀"].map((title) => (
            <div key={title} className="p-5 rounded-3xl bg-white/40 backdrop-blur-sm border border-white/60">
              <h3 className="text-lg font-bold text-[#333] mb-1">{title}</h3>
              <p className="text-sm text-[#333]/50">Optimistic futurism from the year 2000.</p>
            </div>
          ))}
        </div>
      </div>
...
export function CorporateSaaSPreview() {
  return (
    <div className="min-h-full bg-white text-[#1e293b] p-8">
      <div className="max-w-3xl mx-auto">
        {/* Nav */}
        ...
        <div className="text-center mb-12">
          ...
          <p className="text-lg text-[#64748b] max-w-md mx-auto mb-8">
            The all-in-one platform that helps teams build, deploy, and scale their products.
          </p>
        ...
        <div className="grid grid-cols-3 gap-6">
          {[
            { title: "Analytics", desc: "Real-time insights into your data" },
            { title: "Automation", desc: "Workflows that save hours daily" },
            { title: "Security", desc: "Enterprise-grade protection" },
          ].map((feature) => (
            <div key={feature.title} className="p-6 rounded-xl bg-[#f8fafc] border border-[#e2e8f0]">
...
export function EcommercePreview() {
  return (
    <div className="min-h-full bg-[#fafaf9] text-[#1c1917] p-8">
      <div className="max-w-3xl mx-auto">
        <div className="flex items-center justify-between mb-8">
          ...
          <div className="flex gap-6 text-sm text-[#78716c]">
            <span>Shop</span><span>Collection</span><span>About</span>
          </div>
        </div>

        <div className="bg-[#e7e5e4] rounded-2xl p-12 mb-8 text-center">
          <div className="text-xs uppercase tracking-widest text-[#78716c] mb-4">New Arrivals</div>
          <h1 className="text-4xl font-medium mb-4" style={{ fontFamily: "'DM Serif Display', serif" }}>
            The Spring Edit
          </h1>
          <p className="text-[#78716c] mb-6">Curated pieces for the new season.</p>
        ...
        <div className="grid grid-cols-3 gap-4">
          {[
            ...
            <div key={product.name} className="group cursor-pointer">
              <div className="bg-[#e7e5e4] rounded-xl aspect-[3/4] mb-3 flex items-center justify-center text-[#a8a29e] text-sm">
                Image
              </div>
